...
const STORE_ASSET_FIELDS = ['facade_image', 'banner_mobile', 'banner_desktop', 'logo', 'favicon'];
...
export async function loadStore() {
    ...
    try {
        ...
        const rawPhone = motel.phone ?? null;
        const rawWhatsapp = motel.whatsapp ?? null;
        ...
        applyVersionedStoreAssets(store, STORE_ASSET_FIELDS);
        store.phone = formatContactNumber(rawPhone);
        store.whatsapp = formatContactNumber(rawWhatsapp);
        store.whatsapp_link = buildWhatsAppLink(rawWhatsapp);
        ...
        store.menu_background_image = versionAssetUrl(getMostExpensiveSuiteImage(store.suites));
        store.ready = true;
...
function applyVersionedStoreAssets(store, fields) {
    for (const field of fields) {
        store[field] = versionAssetUrl(store[field]);
    }
}
...
function formatContactNumber(value) {
    ...
    const digits = normalizeContactDigits(text);
    if (digits.length === 10) {
...
function parsePriceValue(value) {
    ...
    const decimalString =
        decimalSeparatorIndex > -1
            ? `${normalized.slice(0, decimalSeparatorIndex).replace(/[.,]/g, '')}.${normalized
