...
const app = createApp({
    setup() {
        ...
        const topoStyle = computed(() => buildBackgroundStyle(store.facade_image));
...
app.component('gm-header', {
    setup() {
        ...
        const menuBgStyle = computed(() => buildBackgroundStyle(store?.menu_background_image));
...
for (const componentName of STORE_COMPONENTS) {
    await registerStoreComponent(app, componentName);
}

app.mount('#app');

Vue.nextTick(() => loadDeferredScripts());
...
async function registerStoreComponent(app, name) {
    app.component(name, {
        setup() {
...
function parseJsonArrayOrCsv(value) {
    ...
    try {
        ...
        // Preserve support for legacy comma-separated values.
    }
...
function periodLabel(period) {
    return Number(period) === 1 ? 'Hora' : 'Horas';
}
